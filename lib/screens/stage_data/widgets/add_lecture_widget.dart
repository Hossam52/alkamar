...
class _AddLectureWidgetState extends State<AddLectureWidget> {
  ...
  Widget build(BuildContext context) {
    return StageBlocConsumer(listener: (context, state) {
      if (state is CreateLectureErrorState) {
        Methods.showSnackBar(context, state.error);
      }
      if (state is CreateLectureSuccessState) {
        Methods.showSuccessSnackBar(context, 'تم اضافة المحاضرة بنجاح');
            ...
            child: SingleChildScrollView(
              child: Column(
                ...
                children: [
                  ...
                  if (state is CreateLectureLoadingState)
                    const DefaultLoader()
                  else
                    CustomButton(
                      text: 'اضف محاضرة',
                      onPressed: () {
                        if (formKey.currentState!.validate()) {
                          StageCubit.instance(context).createLecture(
